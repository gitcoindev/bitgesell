...
class BGLTestMetaClass(type):
    """Metaclass for BGLTestFramework.

    Ensures that any attempt to register a subclass of `BGLTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'BGLTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("BGLTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("BGLTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class BGLTestFramework(metaclass=BGLTestMetaClass):
    """Base class for a BGL test script.

    Individual BGL test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use BGL-cli instead of RPC for all commands")
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: BGLds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("BGLRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a BGLd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple BGLd test nodes"""
        for node in self.nodes:
