...
def TaprootSignatureMsg(txTo, spent_utxos, hash_type, input_index = 0, scriptpath = False, script = CScript(), codeseparator_pos = -1, annex = None, leaf_ver = LEAF_VERSION_TAPSCRIPT):
    ...
    if in_type != SIGHASH_ANYONECANPAY:
        ss += BIP341_keccak256_prevouts(txTo)
        ss += BIP341_keccak256_amounts(spent_utxos)
        ss += BIP341_keccak256_scriptpubkeys(spent_utxos)
        ss += BIP341_keccak256_sequences(txTo)
    if out_type == SIGHASH_ALL:
        ss += BIP341_keccak256_outputs(txTo)
    spend_type = 0
