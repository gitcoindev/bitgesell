...
def assert_fee_amount(fee, tx_size, feerate_BGL_kvB):
...
def get_fee(tx_size, feerate_bgl_kvb):
    """Calculate the fee in BGL given a feerate is BGL/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_bgl_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return satoshi_round(target_fee_sat / Decimal(1e8)) # Truncate BGL result to nearest sat
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "BGL.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "BGL.conf")):
        with open(os.path.join(datadir, "BGL.conf"), 'r', encoding='utf8') as f:
            for line in f:
