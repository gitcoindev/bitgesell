...
class PSBTTest(BGLTestFramework):
    ...
    def run_test(self):
        # Create and fund a raw tx for sending 10 BGL
        psbtx1 = self.nodes[0].walletcreatefundedpsbt([], {self.nodes[2].getnewaddress():10})['psbt']

        # If inputs are specified, do not automatically add more:
        utxo1 = self.nodes[0].listunspent()[0]
        assert_raises_rpc_error(-4, "Insufficient funds", self.nodes[0].walletcreatefundedpsbt, [{"txid": utxo1['txid'], "vout": utxo1['vout']}], {self.nodes[2].getnewaddress():240})

        psbtx1 = self.nodes[0].walletcreatefundedpsbt([{"txid": utxo1['txid'], "vout": utxo1['vout']}], {self.nodes[2].getnewaddress():240}, 0, {"add_inputs": True})['psbt']
        ...
        self.log.info(psbtx_info)
        decoded_psbt = self.nodes[0].decodepsbt(psbtx_info["psbt"])
        self.log.info(decoded_psbt)
